...
on(document, 'click', '#btnBuscar', (e) =>{
    e.preventDefault();
    const inputEmail = document.querySelector("#emailBuscar").value;
    const enpointEmail = `/email/${inputEmail}`;
    fetch(url+enpointEmail, {
        method: 'GET'
    })
    .then(res => res.json())
    .then(res => contenedor.innerHTML = tabelHtml(res));
});